	...
	function OnSlain()
	{
		if (Property.Possessed(self, "MAX_HP"))
		{
			if (Property.Get(self, "MAX_HP") <= 0)
			{
				...
				// if it exists and this object inherits from the "FreshZombie"
				// archetype.
				if (Object.InheritsFrom(self, Object.Named("FreshZombie"))
					&& Object.Exists(Object.Named("M-FreshZombieParts")))
				{
					RemoveZP();
					Object.AddMetaProperty(self,
						Object.Named("M-FreshZombieParts"));
				}
				// Add the "M-FemZombieParts" metaproperty to this object
				// if it exists and this object inherits from the "FemZombie"
				// archetype.
...
class T2FRopeToVine extends SqRootScript
{
	...
	function OnSim()
	{
		if (message().starting)
		{
			foreach (l in Link.GetAll(LinkTools.LinkKindNamed("Contains"),
				self))
			{
				if (Object.InheritsFrom(LinkDest(l), Object.Named("RopeArrow")))
				{
					...
					// Destroy the rope arrow object and add the new vine arrow
...
class T2FWeaponFix extends SqRootScript
{

	// Set the attacking state.
	function OnFrobInvBegin() { SetData("Attacking", message().Abort ? 0 : 1); }
	function OnFrobInvEnd() { SetData("Attacking", 0); }
	function OnFrobToolBegin() { SetData("Attacking", 1); }
	function OnFrobToolEnd() { SetData("Attacking", 0); }

	// Ensure the attack has finished when the weapon is deselected.
	function OnInvDeSelect()
	{
		if (IsDataSet("Attacking") && GetData("Attacking") == 1)
		{
			local equipped = Weapon.IsEquipped(message().Frobber, self);
			if (!equipped)
				Weapon.Equip(self, Object.Archetype(self) == ObjID("Blackjack")
					? eDarkWeaponType.kDWT_BlackJack
					: eDarkWeaponType.kDWT_Sword);
			Weapon.FinishAttack(message().Frobber, self);
			if (!equipped)
				Weapon.UnEquip(self);
		}
		SetData("Attacking", 0);
